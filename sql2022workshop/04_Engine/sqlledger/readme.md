...
1. Let's verify the current state of the ledger. Edit the script **verifyledger.sql** by substituting the JSON value result from the **generatedigest.sql** script (include the brackets inside the quotes). Note the **last_verified_block_id** matches the digest and the result in getledgerblocks.sql.
...
1. Generate another digest by executing **generatedigest.sql**. **Save the output value again (including the braces) to be used for verifying the ledger.**
1. Look at the ledger blocks generated by executing the script **getledgerblocks.sql**.
1. Let's verify the ledger again. Edit the script **verifyledger.sql** by substituting the JSON value result from the **generatedigest.sql** script (include the brackets inside the quotes). Note the **last_verified_block_id** matches the digest and the result in getledgerblocks.sql.

    I now know the ledger is verified as of the time the digest was captured both before and after the update. By using this digest I know that:
    ...
    - I also know that no one has tampered with the database ledger blocks as the digest is an **independent** verification of any data stored in SQL Server.
